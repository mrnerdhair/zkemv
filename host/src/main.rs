...
fn do_apdu(card: &pcsc::Card, mut cmd: apdu::Command) -> Result<Result<Vec<u8>, APDUError>, anyhow::Error> {
    if cmd.le.is_none() { cmd.le = Some(0) };
...
fn do_card_things(reader_index: usize, nonce_getter: impl FnOnce([u8; 32]) -> Result<u32>) -> Result<CardThings> {
    ...
    let mut card_missing_message_shown = false;
    let card = loop {
        match ctx.connect(&readers[reader_index], pcsc::ShareMode::Shared, Protocols::ANY) {
            Ok(x) => break x,
            Err(pcsc::Error::NoSmartcard) => {
                if !card_missing_message_shown {
                    println!("Insert or tap card now");
                    card_missing_message_shown = true;
                }
